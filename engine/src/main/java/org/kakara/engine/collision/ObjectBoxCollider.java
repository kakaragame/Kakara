...
public class ObjectBoxCollider implements Collider {
    ...
    public void update() {
        if(isTrigger) return;
        this.deltaPosition = new Vector3(item.getPosition()).subtract(this.lastPosition);
        this.lastPosition = new Vector3(item.getPosition());

        CollisionManager cm = handler.getCollisionManager();
        for(GameItem gi : cm.getCollidngItems()){
            if(gi == item) continue;
            if(cm.isColliding(gi, item)){
                Vector3 currentPosition = new Vector3(item.getPosition()).subtract(deltaPosition);
